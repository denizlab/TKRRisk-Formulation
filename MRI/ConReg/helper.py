...
def train_siam(train_loader, val_loader,yr,val_df, model,optimizer,scheduler,loss_fn,loss_fn_1,experiment,gamma,result_path,dropout,num_epoch=2, print_every=100,device="cpu"):
    ...
    for epoch in range(num_epoch):
        ...
        for i, (data, labels1,labels2,siam_labels) in enumerate(train_loader):
            ...
            s_loss = loss_fn(hid1.squeeze(),hid2.squeeze(), siam_labels.squeeze())
            loss = gamma*(loss_fn_1(out1.squeeze(), labels1.squeeze())+loss_fn_1(out2[(labels2!=-1).squeeze()].squeeze(), labels2[(labels2!=-1).squeeze()].squeeze())) + s_loss
                
            
            truths.extend(labels2.squeeze())
        ...
        #auc_train = roc_auc_compute_fn(torch.sigmoid(predictions), truths)
        experiment.log_metric('epoch_prog_loss_train', train_siam_loss/total, step=step)
    ...
    # Evaluate after every epoch
        _,_,predictions_0_1,truths_0,predictions_1,truths = evaluate_siam(model,val_loader,device)
