...
class Encoder(nn.Module):
    def __init__(self,model_core,num_ftrs,hidden_dim=64,layer_dim=1,output_dim=1,dropout=0.1,device = "cpu"):
        ...
        for i in range(int(layer_dim)-1):
            if i==0:
...
def get_model(tl_model,hidden_dim=512,layer_dim=1,dropout=0.1,output_dim=1,device="cpu",types="baseline"):
    # load the pretrained model, Resnet34 was used in the paper
    if tl_model == "Resnet18":
        model_ft = model_ft = generate_model(18,n_input_channels=1,conv1_t_stride=2)
    ...
    model_ft = Siam_encoder(model_ft, num_ftrs)
