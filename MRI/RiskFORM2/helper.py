...
def train_siam(train_loader, val_loader,yr,val_df,model1,model2,optimizer,scheduler,loss_fn,loss_fn_1,experiment,gamma,result_path,dropout,num_epoch=2, print_every=100,device="cpu"):
    # Training steps
    start_time = time.time()
    ...
    for epoch in range(num_epoch):
        ...
        for i, (data, labels1,labels2,siam_labels) in enumerate(train_loader):
            ...
            optimizer.zero_grad()
            out1= model1(data[:,0,:,:,:,:].float())
            out2= model2(data[:,1,:,:,:,:].float())
            ...
            actual_out1 = m(out1.squeeze())
            actual_out2 = 1-(1-m(out1.squeeze()))*m(out2.squeeze())
            ...
            if ((siam_labels==-1)*1.0).mean()!=1:
                loss = (loss_fn_1(actual_out1.squeeze(), labels1.squeeze())+loss_fn_1(actual_out2[(siam_labels!=-1).squeeze()].squeeze(), labels2[(siam_labels!=-1).squeeze()].squeeze())) 
                if (labels2[(siam_labels!=-1).squeeze()].shape == torch.Size([])):
                    truths.append(labels2[(siam_labels!=-1).squeeze()].unsqueeze(0))
                else:
                    truths.append(labels2[(siam_labels!=-1).squeeze()])

                if (actual_out2[(siam_labels!=-1).squeeze()].shape == torch.Size([])):
                    predictions.append(actual_out2[(siam_labels!=-1).squeeze()].unsqueeze(0))
                else:
                    predictions.append(actual_out2[(siam_labels!=-1).squeeze()])
                ...
                loss = loss_fn_1(actual_out1.squeeze(), labels1.squeeze()) #+ gamma*s_loss
            ...
            predictions_0_1.append(actual_out1)
            truths_1.append(Variable(labels1))
        ...
        print(predictions.shape,truths.shape,"check these two")

        auc_train_1 = roc_auc_compute_fn(torch.tensor(predictions).cpu(),torch.tensor(truths).cpu())
        #auc_train = roc_auc_compute_fn(torch.sigmoid(predictions), truths)
    ...
    # Evaluate after every epoch
        _,predictions_0_1,truths_0,predictions_1,truths = evaluate_all_base(model1,model2,val_loader,device)
        ...
        if auc_val > best_auc:
            best_auc = max(auc_val, best_auc)
            wait=0
            torch.save(model1.state_dict(), result_path + "best_auc_model1.prm")
            torch.save(model2.state_dict(), result_path + "best_auc_model2.prm")

        if val_loss <best_val_loss:
            best_val_loss = min(val_loss, best_val_loss)
            wait=0
            
            torch.save(model1.state_dict(), result_path + "best_loss_model1.prm")
            torch.save(model2.state_dict(), result_path + "best_loss_model2.prm")
        print('BEST AUC :',best_auc,', wait: ',wait)
...
def get_metrics(val_df,yr):
    ...
    for i in range(val_df.shape[0]):
        ...
        if loc["last_month"]=="00m":
            auc_pred.append((loc[yr+"Pred_0"]))
            auc_truth.append((loc[yr+"Label0"]))
        else:
            auc_pred.extend([loc[yr+"Pred_0"],loc[yr+"Pred_1"]])
    ...
    val_loss = log_loss(auc_truth,auc_pred)
...
def evaluate(model,val_loader,loss_fn,device):
    ...
    with torch.no_grad():
        ...
        return auc_val,acc
def train_all_base(train_loader, val_loader,model,optimizer,scheduler,loss_fn,experiment,result_path,dropout,num_epoch=2, print_every=100,device="cpu"):
    # Training steps
    start_time = time.time()
    ...
    # Evaluate after every epoch
        ...
        with torch.no_grad():
            for i, (data, labels1,labels2,siam_labels) in enumerate(val_loader):
                ...
                loss = loss_fn(output.squeeze(), all_labels.squeeze())
                val_loss+=loss.item()*all_labels.size(0)
                total+=all_labels.size(0)
                truths.extend(Variable(all_labels).squeeze())
                predictions.extend(output.squeeze())
...
def evaluate_all_base(model1,model2,val_loader,device):
    correct = 0
    total = 0
    model1.eval()
    model2.eval()
    ...
    with torch.no_grad():
        for i, (data, labels1,labels2,siam_labels) in enumerate(val_loader):
            ...
            truths.extend(Variable(labels2))
            truths_0.extend(Variable(labels1))
            predictions_1.extend(actual_out2)
            predictions_0_1.extend(actual_out1)
    ...
    return 0,predictions_0_1,truths_0,predictions_1,truths
