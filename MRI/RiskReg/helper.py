...
def train_siam(train_loader, val_loader,yr,val_df, model,optimizer,scheduler,loss_fn,loss_fn_1,experiment,gamma,result_path,dropout,num_epoch=2, print_every=100,device="cpu"):
    # Training steps
    start_time = time.time()
    ...
    for epoch in range(num_epoch):
        ...
        for i, (data, labels1,labels2,siam_labels) in enumerate(train_loader):
            ...
            if ((siam_labels!=-1)*1.0).sum()!=0:
                s_loss = loss_fn(torch.log(torch.sigmoid(out1[(siam_labels!=-1).squeeze()])),
                    torch.log(torch.sigmoid(out2[(siam_labels!=-1).squeeze()])),
                     -1*torch.ones_like(out1[(siam_labels!=-1).squeeze()]))
                loss = (loss_fn_1(out1.squeeze(), labels1.squeeze())+loss_fn_1(out2.squeeze(), labels2.squeeze())) + gamma*s_loss
                try:
                    truths.extend(labels2)
                    predictions.extend(out2.squeeze())
                except:
                    truths.append(labels2)
                    predictions.append(out2.squeeze())
            else:
                loss = loss_fn_1(out1.squeeze(), labels1.squeeze()) +loss_fn_1(out2.squeeze(), labels2.squeeze())
...
def train_all_base(train_loader, val_loader,model,optimizer,scheduler,loss_fn,experiment,result_path,dropout,num_epoch=2, print_every=100,device="cpu"):
    ...
    for epoch in range(num_epoch):
        ...
        for i, (data, labels1,labels2,siam_labels) in enumerate(train_loader):
            ...
            optimizer.zero_grad()
            all_data = torch.cat([data[:,0,:,:,:,:],data[:,1,:,:,:,:]],0)

            #print(all_data.shape,"all data shape")
            all_labels = torch.cat([labels1,labels2],0)
            #print("here")
            output= model(all_data.float())
            #print("here 2")
