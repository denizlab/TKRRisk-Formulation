...
def main() -> None:
    ...
    if args.result_path is None:
        ...
        result_path = result_path + "/"+config.tl_model+"_model_"+"epochs_"+str(config.num_epoch)+"_lr_"+str(config.learning_rate)+"_ntimes_"+str(config.n_times)+"_layer_dim_"+str(config.layer_dim)+"_hidden_dim_"+str(config.hidden_dim)+"_imbtype_"+config.imb_mode+"_weight_decay_"+str(config.weight_decay)+"_setting_"+str(config.setting)+"_type_"+str(config.types)+"_gamma_"+str(config.gamma)+"_margin_"+str(config.margin)+"_pretrained_"+str(config.pretrained)+"/"
    else:
        result_path = args.result_path


    csv_path = "/gpfs/data/denizlab/Users/hrr288/data/1yrcsvs/"

    if args.dataset == "OAI":
        test_df = pd.read_csv(csv_path+"val.csv",index_col=0)
        test_data = Radiographloader(test_df,image_size,cropped_size,last_index=2,mode = 'val',setting=setting,types=types)
        test_loader = DataLoader(test_data, batch_size=batch_size,shuffle=False)

    if args.dataset == "MOST":
        test_df = pd.read_csv(csv_path+"Testset_MOST.csv")
        test_data = MOSTloader(test_df,image_size,cropped_size,last_index=2,mode='val')
        test_loader = DataLoader(test_data, batch_size=batch_size,shuffle=False)
    ...
    test_df.to_csv(result_path+"result_"+args.dataset+"_"+args.metric+"_df.csv")
