...
def train_siam(train_loader, val_loader,yr,val_df, model,optimizer,scheduler,loss_fn_1,experiment,gamma,result_path,dropout,num_epoch=2, print_every=100,device="cpu"):
    # Training steps
    start_time = time.time()
    ...
    for epoch in range(num_epoch):
        ...
        for i, (data, labels1,labels2,siam_labels) in enumerate(train_loader):
            ...
            optimizer.zero_grad()
            out1= model(data[:,0,:,:,:,:].float())
            out2= model(data[:,1,:,:,:,:].float())
            ...
            actual_out2 = 1-(1-m(out1.squeeze()))*(1-m(out2.squeeze()))
...
def evaluate_siam(model,val_loader,device):
    ...
    with torch.no_grad():
        for i, (data, labels1,labels2,siam_labels) in enumerate(val_loader):
            ...
            out1,out2,hid1,hid2 = model(data.float())
            labels1 = labels1.type_as(out1)
            labels2 = labels2.type_as(out1)
            siam_labels = siam_labels.type_as(out1)
            #print(pred,outputs,labels)
