...
def train_siam(train_loader, val_loader,yr,val_df,model,optimizer,scheduler,loss_fn,loss_fn_1,experiment,gamma,result_path,dropout,num_epoch=2, print_every=100,device="cpu"):
    ...
    # Evaluate after every epoch
        _,_,predictions_0_1,truths_0,predictions_1,truths = evaluate_siam(model,loss_fn,val_loader,device)
...
def train_all_base(train_loader, val_loader,model,optimizer,scheduler,loss_fn,experiment,result_path,dropout,num_epoch=2, print_every=100,device="cpu"):
    ...
    for epoch in range(num_epoch):
        ...
        _,_,_,predictions_0_1,truths_0,predictions_1,truths = evaluate_all_base(model,val_loader,device)
...
def evaluate_siam(model,loss_fn,val_loader,device):
    correct = 0
    total = 0
    model.eval()
    ...
    with torch.no_grad():
        for i, (data, labels1,labels2,siam_labels) in enumerate(val_loader):
            ...
            s_loss = loss_fn(hid1.squeeze(),hid2.squeeze(), siam_labels.squeeze())
            total_siam_loss+=s_loss.item()*labels1.size(0)
    ...
    return 0,total_siam_loss/total,predictions_0_1,truths_0,predictions_1,truths
