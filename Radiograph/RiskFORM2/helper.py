...
def train_siam(train_loader, val_loader,yr,val_df,model1,model2,optimizer,scheduler,loss_fn,loss_fn_1,experiment,gamma,result_path,dropout,num_epoch=2, print_every=100,device="cpu"):
    ...
    for epoch in range(num_epoch):
        ...
        for i, (data, labels1,labels2,siam_labels) in enumerate(train_loader):
            ...
            if (((labels2==-1)&(siam_labels==-1))*1.0).mean()!=1:
                loss = (loss_fn_1(actual_out1.squeeze(), labels1.squeeze())+loss_fn_1(actual_out2[(labels2!=-1).squeeze() & (siam_labels!=-1).squeeze()].squeeze(), labels2[(labels2!=-1).squeeze() & (siam_labels!=-1).squeeze()].squeeze())) 
                try:
                    truths.extend(labels2[(labels2!=-1).squeeze() & (siam_labels!=-1).squeeze()])
                    predictions.extend(actual_out2[(labels2!=-1).squeeze() & (siam_labels!=-1).squeeze()].squeeze())
                except:
                    truths.append(labels2[(labels2!=-1).squeeze() & (siam_labels!=-1).squeeze()])
                    predictions.append(actual_out2[(labels2!=-1).squeeze() & (siam_labels!=-1).squeeze()].squeeze())
            else:
            ...
            predictions_0_1.extend(actual_out1.squeeze())
        ...
        auc_train_1 = roc_auc_compute_fn(torch.tensor(predictions),torch.tensor(truths))
        #auc_train = roc_auc_compute_fn(torch.sigmoid(predictions), truths)
    ...
    return model1,model2, loss_train, val_loss,best_auc
